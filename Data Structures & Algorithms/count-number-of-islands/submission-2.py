        ...
        while q:
            ...
            for neighbor in neighbors:
                ...
                if neighbor not in lands and grid[x][y] == "1":
                    lands.add(neighbor)
                    q.append(neighbor)
        # if we have more unvisited land cells, that means we found new island
        return starting_count < len(lands)
        ...
        for r in range(len(grid)):
            for c in range(len(grid[0])):
                ...
                if grid[r][c] == "1" and cell not in lands:
                    new_island = self.bfs(grid, cell, lands)
                    num_islands += 1 if new_island else 0
